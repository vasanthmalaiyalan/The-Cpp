...
public:
    ...
    // Example: serious hardware/buffer error occurred
    bool bad()  const { return state & badbit;  }

    // good() → returns true only when state is 0 (no bits set)
...
// ─────────────────────────────────────────
// Derived class: inherits all state functions
...
int main()
{
    ...
    std::cout << "\n=== Test 7: MyFileStream - file opened ok ===\n";
    fs.open("data.txt"); // valid name → good
    fs.printState();
    // good()=1, all others 0
...
| Function | Return | அர்த்தம் |
|----------|--------|---------|
| `good()` | `1` | எல்லாம் சரி |
| `eof()` | `1` | Input முடிந்தது |
| `fail()` | `1` | தவறான type |
| `bad()` | `1` | Serious error |

`MyFileStream` தனியாக எழுதவில்லை — **inherit மட்டும் செய்தது**, ஆனால் எல்லா functions-உம் வேலை செய்கின்றன. இதுதான் `cin`, `ifstream` எல்லாமே ஒரே மாதிரி `eof()`, `fail()` வேலை செய்வதன் உண்மையான காரணம்.

-----------------------------------------

``class MyFileStream : public MyStream`இதற்கு என்ன அர்த்தம்`

## `class MyFileStream : public MyStream`

இதன் அர்த்தம்: **MyFileStream என்பது MyStream-இன் child class**
...
// Child class (Derived)
// MyFileStream : public MyStream
// "MyFileStream is a MyStream" என்று படிக்கலாம்
class MyFileStream : public MyStream
{
public:
    void open() { ... } // புதுசா சேர்த்தது மட்டும்
    // eof(), fail(), good(), clear() — தானாக வந்துவிடும்!
};
```
...
fs.open("data.txt"); // MyFileStream-இல் எழுதியது
fs.eof();            // MyStream-இல் இருந்து வந்தது
fs.fail();           // MyStream-இல் இருந்து வந்தது
fs.good();           // MyStream-இல் இருந்து வந்தது
```
...
class MyFileStream : public MyStream
//                    ↑
//              Parent-இன் public functions
//              child-லயும் public-ஆகவே இருக்கும்
```

| Inheritance | Parent public → Child-ல் |
|-------------|--------------------------|
| `public`    | public ஆகவே வரும் ✓     |
| `protected` | protected ஆகி விடும்     |
| `private`   | private ஆகி விடும்       |

பொதுவாக **`public` inheritance மட்டுமே** அதிகம் பயன்படுத்துவார்கள்.

---

## ஒரு வரியில்

> **"MyFileStream என்பது MyStream-இன் எல்லா திறமைகளையும் பெற்று, கூடுதலாக file open/close செய்யும் திறன் சேர்த்துக்கொண்ட class"**